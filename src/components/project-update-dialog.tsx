...
import {FaAlignLeft, FaArrowRight, FaCheck, FaImage, FaTimes} from "react-icons/fa";
import Image from "next/image";
import {Controller, useForm} from "react-hook-form";
import React, {useEffect, useRef, useState} from "react";
import MarkdownIt from "markdown-it";
import {toast} from "sonner";
import {update_new_project} from "@/server/project";
...
export function ProjectUpdateDialog({data}: { data: TProject }) {
    ...
    const onSubmit = async (fieldData: FormValues) => {
        ...
        const payload: TProject = {
            ...
            slogan: fieldData?.slogan,
            description: fieldData?.description,
            technologies: fieldData?.technologies?.split(",").map((technology: string) => technology),
            features: fieldData?.features?.split(",").map((fet: string) => fet),
            frontEndGitRepo: fieldData?.frontEndGitRepo,
            backEndGitRepo: fieldData?.backEndGitRepo,
            liveLink: fieldData?.liveLink
        }
        try {
            ...
            // create new project
            const res = await update_new_project(data?._id as string, formData)
            if (res?.success) {
                toast.success('Project updated successful.', {id});
                setOpenModal(false);
    ...
    useEffect(() => {
        setImagePreview(data?.projectImage as string);
    }, [data?.projectImage])
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="mt-1 flex flex-col items-center">
                                            {imagePreview ? (
                                                <div className="mb-4 relative">
                                                    ...
                                                    >
                                                        <Image
                                                            ...
                                                            width={100}
                                                            height={100}
                                                            className="w-full h-36 object-contain rounded-md border border-gray-300"
                                                        />
                                                        <button
                                                            type="button"
                                                            onClick={() => {
                                                ...
                                                <div
                                                    className={`flex justify-center px-6 pt-5 pb-6 border-2 ${dragActive ? 'border-amber-500 bg-amber-50' : 'border-gray-300 border-dashed'} rounded-md w-full h-36 transition-colors duration-200`}
